<template>
  <div class="">
    <div v-if="m_screenSize == 'lg'" style="display:inline" >
    <v-badge
      class=""
      ...
      <v-menu  class="" offset-y :position-x="10" content-class="menu-toolbar" >
        <template v-slot:activator="{ on }">
            ...
            <v-btn class="menu-btn" outlined tile icon style="height:50px;text-transform: capitalize!important; border:none!important" v-on="on">
       ...
       &nbsp;
      <v-divider  style="border-width: 0.6px!important;display:inline" vertical ></v-divider>
      &nbsp;
      <v-menu offset-y content-class="menu-toolbar" v-if="m_screenSize == 'lg'">
        ...
        <v-list>
          <v-list-item  to="/client/organizations" nuxt link  >
          ...
          <v-list-item to="/client/plans/users" nuxt link exact >
      ...
      <div v-else>
          <v-btn class="menu-btn"  text  nuxt link exact to="/client/notifications">
            <v-icon>mdi-bell</v-icon>
          </v-btn>

          <v-btn  nuxt text link exacto to="/client/profile">
            <img :src="image" style="width:26px;border-radius: 20px;max-height: 30px;" alt="">
          </v-btn>
